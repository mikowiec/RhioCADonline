...
void main() {
    vec3 materialColor = texture(uAlbedoTexture, vTexcoord).rgb;
    float materialMetallic = max(EPSILON, 1.0 - texture(uMetallicTexture, vTexcoord).b);
    vec3 materialSpecular = vec3(1.0 - texture(uRoughnessTexture, vTexcoord).g);
    ...
    for (int i = 0; i < directionalLightsCount; i++) {
        ...
        if (lightAmount > EPSILON) {
            ...
            float specularTerm = specularPhong(L, E, N, 1.0 + materialSpecular.x * 100.0);

            kD += lightAmount * lambertTerm  * materialColor    * (lightColor - ambientColor);
            kS += lightAmount * specularTerm * mix(vec3(1.0), materialColor, materialMetallic) * lightColor;
        }
    }
